...
11. 数组 : 数组的类型是`Array` ,数组中可以存储任意类型的数据，实际上等价于Java中的List，其长度也是自动变化的。
    ...
    arr.reduce(function(prev, currentValue,CurrentIndex,arr){}) 和 arr.reduceRight(function(prev, currentValue,CurrentIndex,arr){}); // 接收一个函数作为累加器，会依次把数组元素传入到函数中，然后把最后一个元素传入函数的返回值作为整体的返回值。Right表示从最后一个元素开始。
